...
void
process_payload(uint8_t *data, size_t data_len, int crc_correct, block_buffer_t *block_buffer_list, int adapter_no) {
    if (!crc_correct) {
    ...
    // if aram_data_packets_per_block+param_fec_packets_per_block would be limited to powers of two, this could be
    // replaced by a logical AND operation
    ...
    //or we have received a block_num that is several times smaller than the current window of buffers -> this indicated
    // that either the window is too small or that the transmitter has been restarted
    ...
    if ((block_num > max_block_num || tx_restart) && crc_correct) {
        ...
        if (last_block_num != -1) {
            ...
            // we have both pointers to the packet buffers (to get information about crc and vadility) and raw data
            ...
            rx_status->received_packet_cnt =
                    rx_status->received_packet_cnt + param_data_packets_per_block + param_fec_packets_per_block -
            ...
            while (di < param_data_packets_per_block && fi < param_fec_packets_per_block) {
                ...
                // if this DATA is corrupt and there are less good fecs than missing datas we cannot do anything for
                // this data
                ...
                // if this FEC is corrupted and there are more lost packages than good fecs we should replace this DATA
                // even with this corrupted FEC
            ...
            fec_decode((unsigned int) param_packet_length, data_blocks, param_data_packets_per_block, fec_blocks,
            ...
            for (i = 0; i < param_data_packets_per_block; ++i) {
                ...
                if (!reconstruction_failed || data_pkgs[i]->valid) {
                    // if reconstruction did fail, the data_length value is undefined. better limit it to some sensible
                    // value
    ...
    if (i != param_block_buffers) {
        ...
        packet_num = wph->sequence_number % (param_data_packets_per_block +
                                             param_fec_packets_per_block); //if retr_block_size would be limited to
        // powers of two, this could be replace by a locical and operation

        // only overwrite packets where the checksum is not yet correct. otherwise the packets are already received
        // correctly
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < param_block_buffers; ++i) {
        ...
        block_buffer_list[i].packet_buffer_list = lib_alloc_packet_buffer_list(
                param_data_packets_per_block + param_fec_packets_per_block, MAX_PACKET_LENGTH);
    }
